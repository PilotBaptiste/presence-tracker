...
const parseTime     = (str) => { if(!str)return 0; const s=String(str).trim(); const c=s.match(/^(\d+):(\d{1,2})$/); if(c)return parseInt(c[1])*60+parseInt(c[2]); const h=s.match(/^(\d+)h(\d{0,2})$/i); if(h)return parseInt(h[1])*60+(h[2]?parseInt(h[2]):0); return 0; };
...
const CustomTooltip = ({active,payload,label}) => {
  ...
  return(<div style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:"12px",padding:"10px 14px",boxShadow:"0 4px 16px rgba(0,0,0,0.1)"}}>
...
function LockModal({onUnlock,onClose}) {
  const [pwd,setPwd]=useState(""),[ err,setErr]=useState(false),[loading,setLoading]=useState(false);
  const tryUnlock=async()=>{ setLoading(true);setErr(false); const h=await hashStr(pwd); if(h===PWD_HASH){onUnlock();}else{setErr(true);setPwd("");} setLoading(false); };
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.5)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999}}>
      <div style={{background:"#fff",borderRadius:"20px",padding:"32px 28px",width:"100%",maxWidth:"360px",boxShadow:"0 20px 60px rgba(0,0,0,0.2)",margin:"16px"}}>
        <div style={{textAlign:"center",marginBottom:"24px"}}>
          ...
          <p style={{fontSize:"13px",color:"#94a3b8",marginTop:"4px"}}>Entre le mot de passe pour modifier les données</p>
        </div>
        <input type="password" placeholder="Mot de passe" value={pwd} onChange={e=>{setPwd(e.target.value);setErr(false);}} onKeyDown={e=>e.key==="Enter"&&tryUnlock()} autoFocus
        ...
        <div style={{display:"flex",gap:"8px",marginTop:"8px"}}>
          ...
          <button onClick={tryUnlock} disabled={loading||!pwd} style={{flex:2,padding:"11px",borderRadius:"10px",border:"none",background:pwd?"#2563eb":"#bfdbfe",color:"#fff",fontWeight:700,fontSize:"14px",cursor:pwd?"pointer":"default"}}>
...
function TimeInput({value,onChange,color="#2563eb",label,editMode}) {
  const [raw,setRaw]=useState(value||""),[focused,setFocused]=useState(false);
  useEffect(()=>{if(!focused)setRaw(value||"");},[value,focused]);
  const display=()=>{ if(focused)return raw; if(!value)return""; const m=parseTime(value); return`${Math.floor(m/60)}:${String(m%60).padStart(2,"0")}`; };
  const handleBlur=()=>{ setFocused(false); const mins=parseTime(raw); if(mins>0){const f=`${Math.floor(mins/60)}:${String(mins%60).padStart(2,"0")}`;setRaw(f);onChange(f);}else{setRaw("");onChange("");} };
  const ro=!editMode;
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"4px"}}>
      <label style={{fontSize:"9px",fontWeight:800,letterSpacing:"0.5px",color:ro?"#94a3b8":color,textTransform:"uppercase"}}>{label}</label>
      <input type="text" placeholder="0:00" value={display()} readOnly={ro}
        onChange={e=>editMode&&setRaw(e.target.value.replace(/[^0-9:h]/gi,""))}
        onFocus={()=>{if(!editMode)return;setFocused(true);setRaw(value||"");}}
        onBlur={handleBlur}
        style={{width:"100%",height:"42px",border:`2px solid ${focused?color:ro?"#e2e8f0":color+"44"}`,borderRadius:"10px",textAlign:"center",fontSize:"14px",fontWeight:700,color:ro?"#94a3b8":color,background:ro?"#f8fafc":color+"10",outline:"none",boxShadow:focused?`0 0 0 3px ${color}22`:"none",transition:"all 0.2s",cursor:ro?"default":"text"}}
      />
    </div>
  );
}
...
function MonthRow({month,mi,year,values,onChange,editMode}) {
  ...
  const flightMins=parseTime(values.flightTime),dutyMins=parseTime(values.dutyTime);
  const sectors=parseInt(values.sectors)||0,onDays=parseInt(values.ON)||0;
  return(
    <div style={{...card(),padding:"16px 18px",border:`1.5px solid ${over?"#fca5a5":ok?"#86efac":"#e2e8f0"}`,transition:"border-color 0.3s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}}>
        ...
        <span style={{fontWeight:900,fontSize:"15px",color:over?"#dc2626":ok?"#16a34a":"#94a3b8"}}>
          {total}<span style={{fontSize:"11px",fontWeight:500,color:"#cbd5e1"}}>/{days}</span>{ok&&" ✓"}{over&&" ⚠️"}
        </span>
      </div>
        ...
        {STATUSES.map(st=>{
          ...
          return(
            <div key={st.key} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"4px"}}>
              <label style={{fontSize:"9px",fontWeight:800,letterSpacing:"0.5px",color:ro?"#94a3b8":st.color,textTransform:"uppercase"}}>{st.label}</label>
              <input type="number" inputMode="numeric" min="0" max={days} value={values[st.key]??""} readOnly={ro}
                onChange={e=>editMode&&onChange(month.name,st.key,e.target.value.replace(/\D/,""))}
                style={{width:"100%",height:"40px",border:`2px solid ${ro?"#e2e8f0":st.color+"33"}`,borderRadius:"10px",textAlign:"center",fontSize:"16px",fontWeight:800,color:ro?"#94a3b8":st.text,background:ro?"#f8fafc":st.bg,outline:"none",cursor:ro?"default":"text",WebkitAppearance:"none",MozAppearance:"textfield",transition:"all 0.2s"}}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"8px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"4px"}}>
          ...
          <input type="number" inputMode="numeric" min="0" value={values.sectors??""} readOnly={!editMode}
...
// ── PresenceCharts ───────────────────────────────────────────────────────────
function PresenceCharts({allData,years,selectedYear,onYearChange}) {
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
      ...
      <div style={{display:"flex",gap:"6px",alignItems:"center",flexWrap:"wrap"}}>
        <span style={{fontSize:"12px",color:"#94a3b8",fontWeight:600}}>Détail par année :</span>
        {years.map(y=>(
          <button key={y} onClick={()=>onYearChange(y)} style={{padding:"5px 12px",borderRadius:"8px",border:`1.5px solid ${selectedYear===y?"#2563eb":"#e2e8f0"}`,background:selectedYear===y?"#eff6ff":"#fff",color:selectedYear===y?"#2563eb":"#94a3b8",fontWeight:700,fontSize:"13px",cursor:"pointer"}}>{y}</button>
        ))}
      </div>
      {allData[selectedYear]&&(
        <CC title={`${selectedYear} — par mois`} sub="Jours de travail · repos · absences">
          <ResponsiveContainer width="100%" height={200}><BarChart data={monthlyData(selectedYear)} barGap={2}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/>
...
// ── FlightStats ───────────────────────────────────────────────────────────────
function FlightStats({allData,years,selectedYear,onYearChange}) {
  ...
  const yearlyStats=years.map(y=>({year:y,totalFlight:MONTHS.reduce((s,m)=>s+ft(allData[y],m.name),0),totalDuty:MONTHS.reduce((s,m)=>s+dt(allData[y],m.name),0),totalSectors:MONTHS.reduce((s,m)=>s+sec(allData[y],m.name),0),totalOn:MONTHS.reduce((s,m)=>s+on(allData[y],m.name),0)}));
  const gF=yearlyStats.reduce((s,y)=>s+y.totalFlight,0),gD=yearlyStats.reduce((s,y)=>s+y.totalDuty,0),gS=yearlyStats.reduce((s,y)=>s+y.totalSectors,0),gO=yearlyStats.reduce((s,y)=>s+y.totalOn,0);
  ...
  const CC=({title,sub,children})=>(<div style={card({padding:"20px"})}><div style={{marginBottom:"16px"}}><h3 style={{fontWeight:800,fontSize:"15px",color:"#1e293b"}}>{title}</h3>{sub&&<p style={{fontSize:"12px",color:"#94a3b8",marginTop:"2px"}}>{sub}</p>}</div>{children}</div>);
  const ys=yearlyStats.find(x=>x.year===selectedYear)||{totalFlight:0,totalDuty:0,totalSectors:0,totalOn:0};
  return(
    <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
      {/* Global KPIs */}
      ...
      {allData[selectedYear]&&(
        <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:"8px"}}>
            <StatCard emoji="✈️" label={`Vol ${selectedYear}`} value={fmtTime(ys.totalFlight)} color="#0891b2" sub={ys.totalOn>0?`moy ${fmtTime(Math.round(ys.totalFlight/ys.totalOn))}/j ON`:undefined}/>
            <StatCard emoji="⏱"  label={`Duty ${selectedYear}`} value={fmtTime(ys.totalDuty)} color="#7c3aed" sub={ys.totalOn>0?`moy ${fmtTime(Math.round(ys.totalDuty/ys.totalOn))}/j ON`:undefined}/>
            <StatCard emoji="🛫" label={`Secteurs ${selectedYear}`} value={ys.totalSectors} color="#16a34a" sub={ys.totalOn>0?`moy ${(ys.totalSectors/ys.totalOn).toFixed(1)}/j ON`:undefined}/>
          </div>
          <CC title={`${selectedYear} — vol & duty par mois`} sub="En heures">
            <ResponsiveContainer width="100%" height={220}><BarChart data={monthlyFlight(selectedYear)} barGap={2}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/>
              <XAxis dataKey="name" tick={{fontSize:11,fill:"#94a3b8"}} axisLine={false} tickLine={false}/>
              <YAxis yAxisId="time" tick={{fontSize:11,fill:"#cbd5e1"}} axisLine={false} tickLine={false} unit="h"/>
              <YAxis yAxisId="sect" orientation="right" tick={{fontSize:11,fill:"#cbd5e1"}} axisLine={false} tickLine={false}/>
          ...
          <CC title={`${selectedYear} — secteurs par mois`} sub="Tronçons volés">
            <ResponsiveContainer width="100%" height={160}><LineChart data={monthlyFlight(selectedYear)}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false}/>
              <XAxis dataKey="name" tick={{fontSize:11,fill:"#94a3b8"}} axisLine={false} tickLine={false}/>
              <YAxis tick={{fontSize:11,fill:"#cbd5e1"}} axisLine={false} tickLine={false}/>
...
export default function App() {
  ...
  const [airportData,setAirportData] = useState({codes:"",extra:{}});
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(ref,snap=>{
      if(snap.exists()){
        ...
        if(d.airports) setAirportData(d.airports);
      }
      setSync("synced");
    },()=>setSync("error"));
    return()=>unsub();
  ...
  const persist=(data,yrs,airports)=>{
    setSync("saving"); clearTimeout(saveTimer.current);
    saveTimer.current=setTimeout(async()=>{
      try{
        await setDoc(doc(db,"users",USER_ID),{allData:data,years:yrs,airports:airports||airportData});
        setSync("synced");
      }catch{setSync("error");}
    },700);
  };
  ...
  const onSaveAirports=(ap)=>{ setAirportData(ap); persist(allData,years,ap); };

  const data=allData[year]||initYearData();
  const sb={loading:{label:"Chargement...",color:"#d97706",bg:"#fffbeb"},saving:{label:"Sauvegarde...",color:"#2563eb",bg:"#eff6ff"},synced:{label:"✓ Synchronisé",color:"#16a34a",bg:"#f0fdf4"},error:{label:"⚠ Erreur",color:"#dc2626",bg:"#fef2f2"}}[sync];
  ...
  return(
    <div style={{background:"#f1f5f9",minHeight:"100vh",fontFamily:"system-ui,-apple-system,sans-serif"}}>
        ...
        <div style={{maxWidth:"920px",margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"10px",flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:"6px",flexWrap:"wrap"}}>
            <div style={{display:"flex",gap:"3px",alignItems:"center"}}>
              ...
              {editMode&&<button onClick={addYear} title="Ajouter une année" style={{padding:"5px 9px",borderRadius:"8px",border:"1.5px dashed #cbd5e1",background:"#fff",color:"#94a3b8",fontWeight:800,fontSize:"14px",cursor:"pointer"}}>＋</button>}
            </div>
      ...
      <div style={{maxWidth:"920px",margin:"0 auto",padding:"14px 16px 60px"}}>
        ...
        {/* Tabs */}
        <div style={{display:"flex",gap:"3px",background:"#e2e8f0",padding:"4px",borderRadius:"12px",width:"fit-content",marginBottom:"16px",flexWrap:"wrap"}}>
          {TABS.map(t=>(
            <button key={t.k} onClick={()=>setTab(t.k)} style={{padding:"7px 13px",borderRadius:"9px",border:"none",cursor:"pointer",fontWeight:700,fontSize:"12px",transition:"all 0.2s",background:tab===t.k?"#fff":"transparent",color:tab===t.k?"#1e293b":"#94a3b8",boxShadow:tab===t.k?"0 1px 4px rgba(0,0,0,0.1)":"none"}}>{t.l}</button>
          ))}
        </div>
        {/* Read-only banner */}
        {!editMode&&(
          <div style={{background:"#f0f9ff",border:"1px solid #bae6fd",borderRadius:"10px",padding:"10px 16px",marginBottom:"14px",display:"flex",alignItems:"center",gap:"10px"}}>
            ...
            <span style={{fontSize:"13px",color:"#0369a1",fontWeight:600}}>Mode lecture seule — clique sur <strong>✏️ Éditer</strong> pour modifier</span>
          </div>
        )}
        {/* Content */}
        {sync==="loading"
          ...
          :<>
            ...
            {tab==="presence" &&<PresenceCharts allData={allData} years={years} selectedYear={chartYear} onYearChange={setChartYear}/>}
            {tab==="flight"   &&<FlightStats allData={allData} years={years} selectedYear={chartYear} onYearChange={setChartYear}/>}
            {tab==="map"      &&<MapTab allData={allData} years={years} editMode={editMode} onSaveAirports={onSaveAirports} airportData={airportData}/>}
          </>
        }
      </div>
      {showLock&&<LockModal onUnlock={()=>{setEditMode(true);setShowLock(false);}} onClose={()=>setShowLock(false)}/>}
    </div>
  );
}